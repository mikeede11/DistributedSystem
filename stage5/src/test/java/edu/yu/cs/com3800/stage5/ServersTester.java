...
public class ServersTester {
    ...
    @Test
    public void testASendMultipleRegularRequestsNoFaults(){
        for (int i = 0; i < ports.length; i++) {
            ...
            try {
                ...
                assertEquals("Hello world! from code version " + i, response.body());
    ...
    @Test
    public void testBKillLeaderBeforeMessagesSentToFollowers(){
        //Kill the leader
        for(ZooKeeperPeerServer server: servers){
            if(server.getServerId() == 7){//this should be leader ID
                server.shutdown();//assassinate
    ...
    @Test
    public void testCKillLeaderAfterMessagesSentToFollowers(){
        for(ZooKeeperPeerServer server: servers){
            if(server.getServerId() == 6){//this should be leader ID
                ((ZooKeeperPeerServerImpl)server).signalToShutdown();//this tells leader to shutdown right after it sends out a request to a follower
                servers.remove(server);
            }
        }
        //send requests
