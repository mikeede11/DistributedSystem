...
public class UDPMessageReceiver extends Thread implements LoggingServer {
    ...
    public UDPMessageReceiver(LinkedBlockingQueue<Message> incomingMessages, InetSocketAddress myAddress, int myPort, ZooKeeperPeerServer peerServer) throws IOException {
        ...
        this.myPort = myPort;
        this.receiverlogger = initializeLogging(this.getClass().getCanonicalName() + "-on-port-" + this.myPort, false);
        this.setDaemon(true);
    ...
    public void run() {
        ...
        try {
            ...
            this.receiverlogger.log(Level.SEVERE, "failed to create receiving socket", e);
            return;
        }
        //loop
        while (!this.isInterrupted()) {
            try {
                this.receiverlogger.fine("Waiting for packet on Peer Server port " + myPort);
                ...
                if (this.peerServer != null && this.peerServer.isPeerDead(sender)) {
                    this.receiverlogger.fine("UDP packet received from dead peer: " + sender.toString() + "; ignoring it.");
                    continue;
                }
                this.receiverlogger.fine("UDP packet received by ps port:" + myPort + "My state is "  + "\n" + received.toString());
                ...
                if (!this.isInterrupted()) {
                    this.receiverlogger.log(Level.WARNING, "Exception caught while trying to receive UDP packet", e);
                }
            }
        }
        //cleanup
    ...
    private void sendElectionReply(byte[] msgContent, InetSocketAddress target) {
        ...
        try (DatagramSocket socket = new DatagramSocket()){
            ...
            this.receiverlogger.fine("Election reply sent:\n" + msg.toString());
        }
        catch (IOException e) {
            this.receiverlogger.warning("Failed to send election reply:\n" + msg.toString());
        }
    }

    /**
     * see if we got an Election LOOKING message while we are in FOLLOWING or LEADING//basically if we have a vote and someone is looking tell them this is who the leader is
     * @param received
     * @return
     */
